...
import { createFromEnvironment, GmailMCPManager, VERSION } from './index.js';
import type { EmailComposition, BatchOperation, EmailListOptions } from './types.js';

const program = new Command();

// Global manager instance
let manager: GmailMCPManager;
  ...
  .action(async () => {
    try {
      ...
      const email: EmailComposition = {
        to: answers.to.split(',').map((email: string) => email.trim()),
      ...
      if (answers.cc && answers.cc.trim()) {
        email.cc = answers.cc.split(',').map((email: string) => email.trim());
      }
