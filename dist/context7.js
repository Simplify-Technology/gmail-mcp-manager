...
export class Context7Service {
     ...
     */
    async getRelevantDocumentation(operation, context) {
        if (!this.enabled) {
            return null;
        }
        try {
     ...
     */
    displayInTerminal(documentation, operation) {
        if (!documentation)
            return;
        console.log('\n' + '='.repeat(80));
     ...
     */
    getAsCodeComments(documentation) {
        if (!documentation)
            return '';
        let comments = '/**\n';
     ...
     */
    mapOperationToSearchTerm(operation, context) {
        const operationMap = {
     ...
     */
    async fetchFromContext7(searchTerm) {
        // Simulate API delay
     ...
     */
    getMockDocumentation(searchTerm) {
        const mockResponses = {
...
4. Attachments (if any) with proper encoding`,
        ...
        const key = Object.keys(mockResponses).find(k => searchTerm.toLowerCase().includes(k.toLowerCase().split(' ').slice(-2).join(' ')));
...
- Follow security best practices for token handling`,
     ...
     */
    formatDocumentationForTerminal(doc) {
        return doc
            .split('\n')
            .map(line => `   ${line}`)
            .join('\n');
